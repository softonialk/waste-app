...
export default function Home() {
  ...
  const [guideCategory, setGuideCategory] = useState<(typeof categories)[number] | null>(null);
  ...
  return (
    <main>
      ...
      <section className="category-section" id="categories"><div className="section">
        ...
        <div className="tip-bar"><span>?</span><div><b>{t("Not sure where your waste belongs?", "ඔබේ කසළ අයත් කාණ්ඩය විශ්වාස නැද්ද?")}</b><p>{t("Use our waste guide to find the correct category.", "නිවැරදි කාණ්ඩය සොයාගැනීමට අපගේ කසළ මාර්ගෝපදේශය භාවිතා කරන්න.")}</p></div><button type="button" onClick={() => { setGuideCategory(null); setWasteGuideOpen(true); }}>{t("Find Waste Category", "කසළ කාණ්ඩය සොයන්න")} <span>→</span></button></div>
        {wasteGuideOpen && <div className="waste-guide-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setWasteGuideOpen(false); }}><div className="waste-guide" role="dialog" aria-modal="true" aria-labelledby="waste-guide-title"><button type="button" className="waste-guide-close" aria-label={t("Close waste guide", "කසළ මාර්ගෝපදේශය වසන්න")} onClick={() => setWasteGuideOpen(false)}>×</button><h3 id="waste-guide-title">{t("What kind of waste do you have?", "ඔබ ළඟ තිබෙන්නේ මොන වගේ කසළද?")}</h3><p>{t("Choose the item that looks most similar.", "වඩාත් ගැළපෙන කසළ වර්ගය තෝරන්න.")}</p><div className="waste-guide-options">{categories.map((category) => <button type="button" className={guideCategory?.title === category.title ? "selected" : ""} onClick={() => setGuideCategory(category)} key={category.title}><Image src={category.image} alt="" width={56} height={56}/><span><b>{isSi ? category.titleSi : category.title}</b><small>{isSi ? category.itemsSi : category.items}</small></span></button>)}</div>{guideCategory && <div className="waste-guide-answer" role="status"><span>✓</span><p><small>{t("Your waste category is", "ඔබේ කසළ කාණ්ඩය")}</small><b>{isSi ? guideCategory.titleSi : guideCategory.title}</b></p></div>}<button type="button" className="waste-guide-done" onClick={() => setWasteGuideOpen(false)}>{t("Done", "හරි")}</button></div></div>}
      </div></section>
